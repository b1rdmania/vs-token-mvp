...
contract MockUpgradeableVault is UpgradeableVault {
    constructor(
        ...
        UpgradeableVault(_vS, _sonicNFT, _underlyingToken, _protocolTreasury, _maturityTimestamp, _vaultFreezeTimestamp)
    {}
...
contract UpgradeableVaultTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory tokenInitData =
            abi.encodeWithSelector(UpgradeableVSToken.initialize.selector, "Test vS Token", "vsTEST", admin);
        ...
        UpgradeableVault vaultImplementation = new UpgradeableVault(
            address(vsToken), address(mockNFT), address(mockToken), treasury, MATURITY_TIMESTAMP, FREEZE_TIMESTAMP
        );

        // Step 5: Deploy vault proxy
        bytes memory vaultInitData = abi.encodeWithSelector(UpgradeableVault.initialize.selector, admin);
    ...
    function testUpgradeIntervalProtection() public {
        ...
        address impl1 = address(
            new MockUpgradeableVault(
                address(vsToken), address(mockNFT), address(mockToken), treasury, MATURITY_TIMESTAMP, FREEZE_TIMESTAMP
            )
        );
        address impl2 = address(
            new MockUpgradeableVault(
                address(vsToken), address(mockNFT), address(mockToken), treasury, MATURITY_TIMESTAMP, FREEZE_TIMESTAMP
            )
        );
    ...
    function testRoleHierarchyAndPermissions() public {
        ...
        vm.prank(user1);
        vm.expectRevert(
            "AccessControl: account 0x0000000000000000000000000000000000001111 is missing role 0xa49807205ce4d355092ef5a8a18f56e8913cf4a201fbe287825b095693c21775"
        );
